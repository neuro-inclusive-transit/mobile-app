<script type="ts">
  ...
  export function navBackwards(toId?: string | undefined) {
    const toPage = toId
      ? Frame.topmost().backStack.find(
          (entry) => entry.resolvedPage.id === toId,
        )
      : undefined;
  ...
  export function navForwards(page?: NavigationOptions["page"]) {
    if (!nextPage && !page) {
      return;
    }
    navigate({
  ...
  <gridLayout columns="*, auto, *" rows="auto, *, auto" class="p-t-m">
    <stackLayout row={0} columnSpan={3} class="main-layout">
      <button
        ...
        class="link text-left m-b-xxs"
      />
      <slot name="header">
        <DepartureDestinationSwitcher
          departure={$planJourney.departure?.name}
          destination={$planJourney.arrival?.name}
          on:switchValues={onSwitchValues}
          on:tapDeparture={onTapDeparture}
          on:tapDestination={onTapDestination}
        />
